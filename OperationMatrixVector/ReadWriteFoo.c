...
void scriviValore( matrice *m, int i, int j )
{
  int res;
     ...
     {
         printf("m[%d][%d] = ", i, j);
         res = scanf("%f", (m->mat+(i*leggiRighe(m)+j)));
         scanf("%*[^\n]");
         if ( res == 0 && *(m->mat+(i*leggiRighe(m)+j)) > FLT_MAX )
            printf("Valore inserito non corretto!!Sono ammessi solo valori reali!\n");

     }while( res == 0 && *(m->mat+(i*leggiRighe(m)+j)) > FLT_MAX );
