...
matrice prodvetMatrice( matrice *elenco, int cont)
{
    ...
    MpvN.id = cont;
    MpvN.mat = (List*)malloc(sizeof(List));
    MpvN.mat->val = 0;

    /*
        NEL PRIMO CICLO CHE ASSEGNA IL VALORE AL PRIMO ELEMENTO
        C'E' QUALCOSA CHE NON QUADRA PERCHE' IL VALORE RIMANE SETTATO A ZERO.
        P.S. SE PROVI A CAMBIARE L'ASSEGNAZIONE QUI SOPRA E METTI UN ALTRO VALORE
        TI RENDERAI CONTO CHE QUEL VALORE PERMANE PER L'INTERA DURATA DELL'OPERAZIONE
        E LA COSA E' ALQUANTO STRANA PERCHE' NOI LAVORIAMO CON LA STESSA FUNZIONE CON TUTTE LE
        MATRICI CHE USIAMO.
        TUTTO E' INIZIALIZZATO QUINDI NON PENSO SIA DOVUTO ALLA MANCATA INIZIALIZZAZIONE DI UN QUALCOSA
        SEMPLICEMENTE, DOVREMMO GESTIRE MEGLIO IL CASO IN CUI IL NODO E' DI TESTA.
        PERCHE' LA POSIZIONE DELLA MATRICE 0,0 CORRISPONDE A TUTTI GLI EFFETTI ALLA TESTA DELLA NOSTRA LISTA
        CHE VIENE SI INIZIALIZZATA E ALLOCATA CORRETTAMENTE, MA SISTEMATICAMENTE, VIENE SALTATA NEL MOMENTO IN CUI
        SI FA ACCESSO ALLA STESSA.
    */
