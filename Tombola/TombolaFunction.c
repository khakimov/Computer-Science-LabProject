...
void printClassifica( ListaGiocatori *listag )
{
    ...
    int i;
    int j, comodo_punt, comodo_id, scambio;
    Giocatore *curr_gioc;
...
void salvaPartita( ListaGiocatori *list, Tombolone *t, ListaPremi p, Estrazione *estr, Impostazioni *imp )
{
  ...
  else
    {
        ...
        save.imp = *imp;
        save.lista = *list;
        memcpy( save.lista_premi, p, sizeof(ListaPremi));
        save.estr = *estr;
...
void caricaPartita( void )
{
    ...
    */
    ...
    else
    {
        ...
        fread( &save, sizeof(Salvataggio), 1, fp );
        estr = save.estr;
        t = save.t;
        list = save.lista;
        memcpy(p, save.lista_premi, 5 * sizeof(Premio));
        imp = save.imp;

        printf("LEGGI PREMIO %s\n", leggiNomePremio(&p[0]));
